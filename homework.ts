...
type newTypeSecond = IdOrName<Id>;
...
function MethodDecorator(enumerable: number) {
    return function (
        target: Object,
        propertyName: string,
        ...
        console.log(
            "I'm Method Decorator!",
            "My target is " + target,
            "My Property is " + propertyName,
            "My propertyDescriptor is " + propertyDescriptor
        );
        propertyDescriptor.value = function (value: number) {
            if (enumerable === 0) {
                return value;
            } else {
                return value * 50;
